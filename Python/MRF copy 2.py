...
class MacroRandomForest:
        ...
        for i in the_seq:
            ...
            if num_first_split >= min_frac_times_no_cols and num_second_split >= min_frac_times_no_cols:
                ...
                if not self.fast_rw:
                    ...
                    everybody = self._random_walk_regularisation(
                        whos_who, id1, 1, nrrd, rando_vec)

                    everybody2 = self._random_walk_regularisation(
                    ...
                    if len(everybody2) == 0 and add_neighbours:
                        yy = np.append(yy, y_neighbors)
                        zz = np.vstack([np.matrix(zz), z_neighbors])

                    elif len(everybody2) == 0 and not add_neighbours:
                        pass # This means we don't add 2-neighbours or 1-neighbours
                ...
                if not self.fast_rw:
                    ...
                    everybody2 = np.array([a for a in everybody2 if not a in everybody])
    ...
    def _random_walk_regularisation(self, whos_who, i_d, n_neighbours, nrrd, rando_vec):

        everybody_n = np.unique(np.concatenate((whos_who.take(
            i_d)+n_neighbours, whos_who.take(i_d)-n_neighbours)))
        everybody_n = np.array([
            a for a in everybody_n if not a in whos_who])
        ...
        for i in range(0, len(leafs)):
            ...
            if len(ind_all) > 0:
                ...
                if self.rw_regul > 0:
                    ...
                    neighbors_2_add = True

                    if len(everybody2) == 0:
                        neighbors_2_add = False
                    ...
                    elif neighbors_2_add == False and neighbors_add == True:
                        zz = np.vstack(
                            [zz, z_neighbors])
